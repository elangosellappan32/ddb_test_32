...
const processUnitData = (units, siteObj, siteKey, months) => {
  ...
  const siteUnits = units.filter(
    (unit) =>
      ...
      String(unit.consumptionSiteId) === String(siteId)
  );
...
const palette = [
  "#1976D2", "#2E7D32", "#F9A825", "#D32F2F", "#7B1FA2",
  "#0288D1", "#388E3C", "#FFA000", "#C2185B", "#512DA8",
];

// ---- Main Component ----
const GraphicalConsumptionReport = () => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        for (const combinedId of siteIds) {
          ...
          try {
            const unitsResponse = await fetchAllConsumptionUnits(companyId, siteId);
            if (!unitsResponse?.data) continue;

            const siteObj = allSites.find(
              (s) =>
                String(s.consumptionSiteId) === String(siteId) &&
                String(s.companyId) === String(companyId)
            );
            if (!siteObj) continue;

            const siteKey = `${siteObj.companyId}_${siteObj.consumptionSiteId}`;
            ...
            if (processedUnits.some((unit) => unit.total > 0)) {
              ...
              availableSitesTemp.set(siteKey, {
                ...
                name: siteObj.name,
                companyId: siteObj.companyId,
                consumptionSiteId: siteObj.consumptionSiteId,
                siteData: siteObj,
              });
            }
          } catch (err) {
...
export default GraphicalConsumptionReport;
